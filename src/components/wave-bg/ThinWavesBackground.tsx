...
import { thinWavesConfig, type ThinWaveConfig, THIN_WAVE_SPEED_MULTIPLIER } from './thinWavesConfig.ts';
    ...
    useEffect(() => {
        ...
        const drawWave = (wave: ThinWaveConfig, baseYOffset: number, waveIndex: number) => {
            ...
            for (let x = startX; x < extendedWidth; x += 2) {
                ...
                const secondaryWave = Math.sin(x * liveFrequency * 1.5 + time * wave.speed * speedMultiplier * 0.7) * 0.3;

                const y =
                    canvas.height / 2 +
                    yOffset +
                    (primaryWave + secondaryWave) * liveAmplitude;
                ctx.lineTo(x, y);
            }
