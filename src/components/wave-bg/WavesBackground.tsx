...
import { wavesConfig, type WaveConfig, WAVE_SPEED_MULTIPLIER } from './wavesConfig.ts';
...
const WavesBackground = () => {
    ...
    const [randomOffsets] = useState(() =>
        wavesConfig.map(() => Math.random() * Math.PI * 2)
    );
    ...
    const drawWave = (
        ...
        const shapeTransition = Math.sin(waveTime * 0.002 * speedMultiplier + waveIndex * 0.2) * 0.5 + 0.5;
        const secondaryShapeType = (shapeType + 1) % 8;

        // ОПТИМИЗАЦИЯ 5: Переиспользуем массивы вместо создания новых
        ...
        for (let x = startX; x < extendedWidth; x += step) {
            ...
            const localDeformation = Math.sin(progress * Math.PI * 8 + waveTime * 0.01 * speedMultiplier + waveIndex) * 0.1;
            ...
            const morphNode1 = Math.sin(progress * Math.PI * 3 - waveTime * 0.004 * speedMultiplier + waveIndex) * liveAmplitude * 0.4;
            const morphNode2 = Math.sin(progress * Math.PI * 5 + waveTime * 0.006 * speedMultiplier - waveIndex * 0.5) * liveAmplitude * 0.25;
            const morphNode3 = Math.sin(progress * Math.PI * 7 - waveTime * 0.005 * speedMultiplier + waveIndex * 1.2) * liveAmplitude * 0.15;
            ...
            const centerY =
                ...
                morphNode1 + morphNode2 + morphNode3 +
                mouseDeformation;
    ...
    useEffect(() => {
        ...
        const ctx = canvas.getContext('2d', {
            ...
            // alpha: false, // Убираем альфа-канал если не нужен
        ...
        // ОПТИМИЗАЦИЯ 11: FPS лимитер (опционально)
        const targetFPS = 60;
        ...
        const animate = () => {
            ...
            const isMouseInsideCanvas = mousePos.current.x >= 0 &&
            ...
            const rotatedMouseX = centerX + (smoothMousePos.current.x - centerX) * Math.cos(-Math.PI / 4) - (smoothMousePos.current.y - centerY) * Math.sin(-Math.PI / 4);
            const rotatedMouseY = centerY + (smoothMousePos.current.x - centerX) * Math.sin(-Math.PI / 4) + (smoothMousePos.current.y - centerY) * Math.cos(-Math.PI / 4);
            ...
            waves.forEach((wave, waveIndex) => {
                const dynamicSpacing = 110 + Math.sin(time * 0.0025 * speedMultiplier + waveIndex + randomOffsets[waveIndex]) * 70;
                const yOffset = (waveIndex - waves.length / 2) * dynamicSpacing;
                drawWave(ctx, canvas, wave, yOffset, waveIndex, time, rotatedMouseX, rotatedMouseY, isMouseInsideCanvas, mouseInfluenceStrength.current);
            });

            time += cappedDelta / 16.67;

            // ОПТИМИЗАЦИЯ 13: Сброс времени чаще (каждую минуту вместо 10)
            if (time > 3600) { // ~60 секунд при 60fps
                time = time % 3600;
            }
