...
const StaticLetters: React.FC = () => {
    ...
    // Трансформируем в смещение с плавностью
    // Начальные позиции ближе к центру: DE на +50, ER на -50
    ...
    const smoothDeX = useSpring(deX, { stiffness: 60, damping: 20, mass: 0.8 });
    const smoothErX = useSpring(erX, { stiffness: 60, damping: 20, mass: 0.8 });
    ...
    useEffect(() => {
        ...
        const observer = new IntersectionObserver(
            ([entry]) => {
                ...
                // Просто обновляем прогресс - анимация обратимая
                exitProgress.set(progress);

                // Включаем CSS анимацию ТОЛЬКО когда прогресс полностью 0
                setShouldAnimate(progress === 0);
            },
            {
    ...
    return (
        ...
        >
            <motion.svg
                ...
                style={{ x: smoothDeX }}
            >
                <path d="M56.8182 147.443H0L24.1477 1.98864H78.6932C93.6553 1.98864 106.155 4.99527 116.193 11.0085C126.231 17.0218 133.357 25.6155 137.571 36.7898C141.785 47.964 142.614 61.2689 140.057 76.7046C137.595 91.572 132.599 104.285 125.071 114.844C117.59 125.402 108.026 133.475 96.3778 139.063C84.7775 144.65 71.5909 147.443 56.8182 147.443ZM45.0284 113.92H59.375C66.572 113.92 72.9877 112.666 78.6222 110.156C84.304 107.599 89.0388 103.338 92.8267 97.3722C96.6619 91.3589 99.4318 83.1439 101.136 72.7273C102.746 62.8788 102.699 55.2794 100.994 49.929C99.2898 44.5313 96.0464 40.7907 91.2642 38.7074C86.482 36.5767 80.303 35.5114 72.7273 35.5114H58.0966L45.0284 113.92Z"
                      fill="white"
                />
                <path d="M122.68 147.443L146.827 1.98864H251.657L246.259 33.8068H180.918L176.941 58.8068H236.884L231.486 90.625H171.543L167.566 115.625H232.623L227.225 147.443H122.68Z"
                      fill="white"
                />
            </motion.svg>
